  ...
  <div class="modal-dialog modal- modal-dialog modal-md" role="document">
                        ...
                        <input type="hidden" name="id" value="" class="edit-id">
                          ...
                          <div class="form-group mb-3">
                              ...
                              <div class="input-group input-group-merge input-group-alternative">
                                  ...
                                  <input name="hargajual" class="form-control edit-hargajual" placeholder="Harga Jual Pastikan Lebih Besar Dari Harga Dasar" type="number" >
                                </div>
    ...
    $(document).ready(function() {
        function validatePrices() {
            var hargadasar = parseFloat($('.edit-hargadasar').val());
            var hargajual = parseFloat($('.edit-hargajual').val());

            if (hargajual < hargadasar || isNaN(hargajual) || isNaN(hargadasar)) {
                $('.edit-hargajual-error').text('Harga jual harus lebih besar dari harga dasar.');
                $('.editProductBtn').prop('disabled', true);
            } else {
                $('.edit-hargajual-error').text('');
                $('.editProductBtn').prop('disabled', false);
            }
        }

        $('.edit-hargadasar, .edit-hargajual').on('input', function() {
            validatePrices();
        });

        $('#productForm').on('submit', function(e) {
            validatePrices();
            if ($('.editProductBtn').prop('disabled')) {
                e.preventDefault();
            }
        });
    });
